...
class DGModel_base(nn.Module):
    ...
    def forward_fe(self, x):
        ...
        x = upsample(x, scale_factor=2)
        x2 = F.interpolate(x2, size=x.shape[2:], mode='bilinear', align_corners=False)  # Ensure same size
        ...
        x = upsample(x, scale_factor=2)
        # x = upsample(x, scale_factor=2)
        x1 = F.interpolate(x1, size=x.shape[2:], mode='bilinear', align_corners=False)  # Ensure same size
