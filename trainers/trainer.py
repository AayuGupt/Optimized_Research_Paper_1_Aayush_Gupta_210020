...
class Trainer(object):
    def __init__(self, seed, version, device):
        ...
        self.log_dir = os.path.join('logs', self.version).replace("\\", "/")
        os.makedirs(self.log_dir, exist_ok=True)

    def log(self, msg, verbose=True, **kwargs):
        if verbose:
            print(msg, **kwargs)
        with open(os.path.join(self.log_dir, 'log.txt').replace("\\", "/"), 'a') as f:
            if 'end' in kwargs:
    ...
    def train_epoch(self, model, loss, train_dataloader, val_dataloader, optimizer, scheduler, epoch, best_criterion, best_epoch):
        ...
        # Checkpoint
        if epoch > 0:
            os.remove(glob(os.path.join(self.log_dir, 'last*.pth').replace("\\", "/"))[0])
        self.save_ckpt(model, os.path.join(self.log_dir, f'last.pth').replace("\\", "/"))
        if current_criterion < best_criterion:
            best_criterion = current_criterion
            best_epoch = epoch
            self.log(f'Epoch {epoch}: saving best model...')
            if epoch > 0:
                os.remove(glob(os.path.join(self.log_dir, 'best*.pth').replace("\\", "/"))[0])
            self.save_ckpt(model, os.path.join(self.log_dir, f'best.pth').replace("\\", "/"))
    ...
    def vis(self, model, test_dataloader, checkpoint=None):
        ...
        os.makedirs(os.path.join(self.log_dir, 'vis').replace("\\", "/"), exist_ok=True)
