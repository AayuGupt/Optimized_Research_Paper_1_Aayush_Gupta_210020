...
class DGTrainer(Trainer):
    ...
    def predict(self, model, img):
        ...
        if h >= ps or w >= ps:
            downsample_factor =2
            img_downsampled = F.avg_pool2d(img, kernel_size=downsample_factor)
            pred_dmap = model(img_downsampled.to(self.device))
            if isinstance(pred_dmap, tuple):
                pred_dmap = pred_dmap[0]
            pred_count = pred_dmap.sum().item() / (self.log_para * downsample_factor**2)
        else:
            pred_dmap = model(img.to(self.device))
            if isinstance(pred_dmap, tuple):
                pred_dmap = pred_dmap[0]
            pred_count = pred_dmap.sum().item() / self.log_para
        return pred_count
