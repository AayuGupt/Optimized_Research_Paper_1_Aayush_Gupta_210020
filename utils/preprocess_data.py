...
def run_sta(origin_dir, save_dir, min_size, max_size):
    if origin_dir.split('/')[-1] == 'part_A':
        ...
        part = 'stb'
    for phase in ['train', 'test']:
        sub_dir = os.path.join(origin_dir, phase+'_data').replace("\\", "/")
        if phase == 'train':
            sub_phase_list = ['train', 'val']
            im_path = os.path.join(sub_dir, 'images').replace("\\", "/")
            im_list = glob(os.path.join(im_path, '*jpg').replace("\\", "/"))
            with open(f'{part}_train.txt') as f:
                train_list = f.readlines()
            with open(f'{part}_val.txt') as f:
                val_list = f.readlines()
            train_list = [os.path.join(im_path, i.strip()).replace("\\", "/") for i in train_list]
            val_list = [os.path.join(im_path, i.strip()).replace("\\", "/") for i in val_list]
            for sub_phase, im_list in zip(sub_phase_list, [train_list, val_list]):
                sub_save_dir = os.path.join(save_dir, sub_phase).replace("\\", "/")
                if not os.path.exists(sub_save_dir):
                    os.makedirs(sub_save_dir)
                for im_path in tqdm(im_list):
                    name = os.path.basename(im_path)
                    ...
                    im_save_path = os.path.join(sub_save_dir, name).replace("\\", "/")
                    im.save(im_save_path)
                    gd_save_path = im_save_path.replace('jpg', 'npy')
                    np.save(gd_save_path, points)
        else:
            sub_save_dir = os.path.join(save_dir, 'test').replace("\\", "/")
            if not os.path.exists(sub_save_dir):
                os.makedirs(sub_save_dir)
            im_path = os.path.join(sub_dir, 'images').replace("\\", "/")
            im_list = glob(os.path.join(im_path, '*jpg').replace("\\", "/"))
            for im_path in tqdm(im_list):
                name = os.path.basename(im_path)
                im, points = generate_data_sta(im_path.replace("\\", "/"), min_size, max_size)
                im_save_path = os.path.join(sub_save_dir, name).replace("\\", "/")
                im.save(im_save_path)
                gd_save_path = im_save_path.replace('jpg', 'npy')
                np.save(gd_save_path, points)
